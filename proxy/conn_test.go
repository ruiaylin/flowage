...
func TestConn_DeleteTable(t *testing.T) {
	...
	c.UseDB("flowage")
	if _, err := c.Execute(`drop table if exists flowage_test_proxy_conn`); err != nil {
		t.Fatal(err)
	}
	c.Close()
}
...
func TestConn_CreateTable(t *testing.T) {
	s := `CREATE TABLE IF NOT EXISTS flowage_test_proxy_conn (
...
func TestConn_Insert(t *testing.T) {
	s := `insert into flowage_test_proxy_conn (id, str, f, e, u, i) values(1, "abc", 3.14, "test1", 255, -127)`
...
func TestConn_Select(t *testing.T) {
	s := `select str, f, e, u, i, ni from flowage_test_proxy_conn where id = 1`
...
func TestConn_Update(t *testing.T) {
	s := `update flowage_test_proxy_conn set str = "123" where id = 1`
	...
	if r, err := c.Execute(`select str from flowage_test_proxy_conn where id = 1`); err != nil {
		t.Fatal(err)
	} else {
...
func TestConn_Replace(t *testing.T) {
	s := `replace into flowage_test_proxy_conn (id, str, f) values(1, "abc", 3.14159)`
	...
	s = `replace into flowage_test_proxy_conn (id, str) values(2, "abcb")`
	...
	s = `select str, f from flowage_test_proxy_conn`
...
func TestConn_Delete(t *testing.T) {
	s := `delete from flowage_test_proxy_conn where id = 100000`
...
func TestConn_Trans(t *testing.T) {
	...
	if _, err := c1.Execute(`insert into flowage_test_proxy_conn (id, str) values (111, "abc")`); err != nil {
		t.Fatal(err)
	}

	if r, err := c2.Execute(`select str from flowage_test_proxy_conn where id = 111`); err != nil {
		t.Fatal(err)
	} else {
		if r.RowNumber() != 0 {
			t.Fatal(r.RowNumber())
	...
	if r, err := c1.Execute(`select str from flowage_test_proxy_conn where id = 111`); err != nil {
		t.Fatal(err)
	} else {
		if r.RowNumber() != 1 {
			t.Fatal(r.RowNumber())
...
func TestConn_LastInsertId(t *testing.T) {
	s := `CREATE TABLE IF NOT EXISTS flowage_test_conn_id (
	...
	r, err := c.Execute(`insert into flowage_test_conn_id (str) values ("abc")`)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := c1.Execute(`drop table if exists flowage_test_conn_id`); err != nil {
		t.Fatal(err)
	}
...
func TestConn_RowCount(t *testing.T) {
	...
	r, err := c.Execute(`insert into flowage_test_proxy_conn (id, str) values (1002, "abc")`)
	if err != nil {
		t.Fatal(err)
	}
