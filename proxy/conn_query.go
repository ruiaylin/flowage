...
import (
	...
	"github.com/ruiaylin/flowage/sqlparser"
...
func (c *Conn) getConn(n *Node, isSelect bool) (co *backend.SqlConn, err error) {
	if !c.needBeginTx() {
		if isSelect {
...
func (c *Conn) getShardConns(isSelect bool, stmt sqlparser.Statement, bindVars map[string]interface{}) ([]*backend.SqlConn, error) {
	...
	conns := make([]*backend.SqlConn, 0, len(nodes))
...
func (c *Conn) executeInShard(conns []*backend.SqlConn, sql string, args []interface{}) ([]*Result, error) {
	var wg sync.WaitGroup
	wg.Add(len(conns))

	rs := make([]interface{}, len(conns))

	f := func(rs []interface{}, i int, co *backend.SqlConn) {
...
func (c *Conn) closeShardConns(conns []*backend.SqlConn, rollback bool) {
...
func (c *Conn) beginShardConns(conns []*backend.SqlConn) error {
	if c.isInTransaction() {
		return nil
	}
...
func (c *Conn) commitShardConns(conns []*backend.SqlConn) error {
	if c.isInTransaction() {
		return nil
	}
